...
class display{
	...
	function ReplaceContent(&$content,$offset=0){
		...
		foreach($file_sections as $section_num => $section_data){
			...
			$replacement .= $this->SectionToContent($section_data,$section_num);
			$replacement .= '</div>';
		}
	...
	function IncludeContent($data){
		...
		switch($type){
			case 'gadget':
			return $this->IncludeGadget($requested);

			case 'special':
			return $this->IncludeSpecial($requested);

			default:
			return $this->IncludePage($requested);
		}
	}
